const formatter = (value: number, digits: number): string => {
  ...
  return item ? (value / item.value).toFixed(digits).replace(regexp, "") +  item.symbol : "0";
};
...
export const formatToHumanReadableWithDigits = (value: number, digits: number) : string => {
