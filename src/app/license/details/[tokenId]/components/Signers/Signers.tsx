...
const SignersComponent: FC<Props> = ({ license, refetch }) => {
  ...
  const handleGenerateSigner = async () => {
    try {
      setLoadingStatus({status: 'loading', label: 'Generating an API key for your developer license'});
    ...
    if (checkIsLicenseOwner(fragment)) {
      return (
        <Button
          ...
          type="button"
          onClick={() => {setSignerToDelete(item.address);}}
    ...
    <div className={"license-details-section"}>
      ...
      <div>
        {!!fragment.signers.nodes.length && (
          <Table
            columns={[{name: 'address', label: 'Signer address'}, {name: 'enabledAt', label: 'Enabled on', render: renderEnabledAt}]}
